...
function startScheduler() {
  ...
  // Every 15 minutes — check who needs a daily reminder
      ...
      for (const profile of (profiles || [])) {
        if (
          ...
          const { data: streak } = await supabase
            .from('streaks')
            .select('last_checked_in, current_streak')
  ...
  cron.schedule('0 9 * * *', async () => {
    try {
      ...
      for (const pair of (partners || [])) {
        const { data: s1 } = await supabase.from('streaks').select('last_checked_in').eq('user_id', pair.user1_id).single()
        const { data: s2 } = await supabase.from('streaks').select('last_checked_in').eq('user_id', pair.user2_id).single()

        const user1Posted = s1?.last_checked_in === today
        const user2Posted = s2?.last_checked_in === today

        if (user1Posted && !user2Posted) {
          await notifyUser(pair.user2_id, '👀 Your partner posted!',
            `${pair.profiles?.name || 'Your partner'} already posted today. Don't let them down!`,
            { action: 'checkin' }
          )
        } else if (user2Posted && !user1Posted) {
          await notifyUser(pair.user1_id, '👀 Your partner posted!',
            `${pair.profiles?.name || 'Your partner'} already posted today. Don't let them down!`,
            { action: 'checkin' }
          )
        }
  ...
  // 11pm daily — missed day follow-up
  ...
  console.log('Scheduler started. Jobs running.')
}
